...
# This program will calculate standard deviation 
# from a series of shots fired with various 
# different handloads.

# First scan in the targets and save as jpg's in 
# the folder ./Targets.  The title of the jpg could
# be the grains of powder if all same on that target.
# Otherwise label the target (ie 1, 2, etc).  

# First we will enter the target names and digitize
# the data using engauge. Check settings - no headers.
# Export as powder charge and close engauge.

# Program then runs Deviation2012.sh to calculate
# standard deviation and save data to file.  
...
dir $Datapath/*.jpg
echo ""
    echo "Please enter the name of the Target file (without .jpg) in $Datapath/ :"
#    echo "(Best to use number of grains in load.)"
read Targetfile
echo "You will now digitize the data points.  "
echo " You must change settings and remove header and use space delimiters."
echo " When finished export using number of grains in load as file name!"
echo " When finished with target close engauge."
sleep 2
engauge $Datapath/$Targetfile.jpg 
echo "Do you have another target? Enter y/n:"
read MoreTargets
done

# This part of the program calculates the standard deviation of a DISTANCE BETWEEN FROM THE MEAN OF A set of
...
echo "*     --------------------------    *" >> $Outfile
echo "*                                   *" >> $Outfile
echo "*   n = $n	File = $Outfile                " >> $Outfile
echo "*   Ave. value of x = $Meanx         " >> $Outfile
echo "*   Ave. value of y = $Meany           " >> $Outfile
echo "*                                   *" >> $Outfile
...
echo "*                                   *" >> $Outfile
echo "* Max. spread is $MaxSpread inches." >> $Outfile
echo "*                                   *" >> $Outfile
echo "*************************************" >> $Outfile
echo "" >> $Outfile
cat $Outfile
