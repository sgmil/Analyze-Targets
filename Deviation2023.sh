...
while read x y 					# read x,y
  do								# loop 2
    ...
    Distz=$(echo "scale=2; sqrt($Diffx2+$Diffy2)" | bc -l)
    echo "Distz =" $Distz
    MaxZdiff=$(echo "scale=0; ($Distz-$MaxDistZ)^2" | bc -l)
    echo "1"
#    if (( $(echo "scale=2; ($MaxZdiff > 0)"  | bc -l) ))
#    if (( $(echo "$MaxZdiff > 0" | bc -l) )); 
    if [ "$(echo "$MaxZdiff > 0" | bc -l)" -eq 1 ]
	    then
            echo "2"
		    MaxDistZ=$(echo "scale =2; sqrt($Distz)^2" | bc -l)
            echo "3"
	    fi
 #   echo $Diffx2 $Diffy2
...
echo "Max distance from center =" $MaxDistZ
echo "" >> $Outfile
...
echo "*                                   *" >> $Outfile
echo "*   Ave. dist. from center = $DistMean           " >> $Outfile
echo "*   Max. dist. from center = $MaxDistZ            " >> $Outfile
echo "*                                   *" >> $Outfile
